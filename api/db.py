...
class DatabaseHandler:
    ...
    def get_prices(self, commodity: Optional[str] = None, limit: int = 100) -> List[dict]:
        ...
        if commodity:
            # Convert commodity to uppercase to match database
            commodity_upper = commodity.upper()
            query = "SELECT * FROM prices WHERE commodity = ? ORDER BY timestamp DESC LIMIT ?"
            df = pd.read_sql_query(query, conn, params=(commodity_upper, limit))
        else:
    ...
    def get_historical_data(self, commodity: str, days: int = 30) -> pd.DataFrame:
        ...
        df = pd.read_sql_query(query, conn, params=(commodity_upper, days * 24))  # Assuming hourly data
        conn.close()
