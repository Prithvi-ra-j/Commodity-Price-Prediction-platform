...
class CommodityPredictor:
    ...
    def _load_existing_models(self):
        ...
        for commodity in ['gold', 'silver', 'oil', 'gas']:
            ...
            scaler_file_lower = os.path.join(self.model_path, f"{commodity}_scaler.npy")
            scaler_file_upper = os.path.join(self.model_path, f"{commodity.upper()}_scaler.npy")
            
            # Use uppercase files if they exist, otherwise try lowercase
            model_file = model_file_upper if os.path.exists(model_file_upper) else model_file_lower
            scaler_file = scaler_file_upper if os.path.exists(scaler_file_upper) else scaler_file_lower
    ...
    def train_model(self, commodity: str, df: pd.DataFrame, epochs: int = 50) -> dict:
        ...
        try:
            ...
            # Save model and scaler (use uppercase for consistency)
            model_file = os.path.join(self.model_path, f"{commodity.upper()}_lstm_pytorch.pth")
            scaler_file = os.path.join(self.model_path, f"{commodity.upper()}_scaler.npy")
